    ...
    public ResponseEntity<Pedido> actualizarPedido(@PathVariable Integer id, @RequestBody Pedido p) {
        ...
        if (pe != null) {
            try {
                ...
                pe.setPreciocomplementos(p.getPreciocomplementos());
                pe.setPreciototal(p.getPreciototal());
                pe.setObservacion(p.getObservacion());
                pe.setComprobantedepago(p.getComprobantedepago());
